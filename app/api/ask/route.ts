...
export async function POST(request: Request) {
  try {
    ...
    if (runtimeEnv.OPENAI_API_KEY) {
      const ai = await answerWithOpenAI(question, ranked, citations, runtimeEnv);
      if (ai) {
    ...
    return NextResponse.json({
      answer: buildExtractiveAnswer(question, citations, body.mode === "summary"),
...
async function answerWithOpenAI(
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      instructions:
        "你是一名严格的资料研究助手。只能依据提供的来源回答，不得补充外部知识。每个事实性结论后必须使用 [S1] 形式引用。证据不足时明确说明。回答使用中文，先给直接结论，再给要点。不要编造不存在的来源编号。",
      input: `用户问题：${question}\n\n可用来源：\n${sourceText}`,
...
function buildExtractiveAnswer(question: string, citations: Citation[], isSummary: boolean) {
  if (!citations.length) return "当前资料不足，无法回答这个问题。";
